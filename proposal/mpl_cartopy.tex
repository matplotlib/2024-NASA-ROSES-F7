...
Because each tick is independent it makes it extremely difficult to place a label
...
three collections -- one each for the tick labels, the ticks, and the grid lines --
essentially transposing the representation which will greatly reduce the
overhead at creation time and enable us to leverage more efficient drawing
...
The second change to the library is to generalize
...
style changes to the \texttt{Artist}s.  The second change will
allow support for \texttt{Artist} to trivially participate in the standard automatic
legend support.
...
implementation and back-compatilibilty for our users will add to the development time.
As with the tick work described above
...
missions that would fall within the scope of this grant to implement as well
as the two explicit tasks identified above.
...
% GML: As far as I know there is no formal governance structure for Cartopy.
% Cartopy was started out of the UK Met Office and has been maintained by a
% small group of developers since then. It was recently relicensed from LGPL
% to BSD-3 clause, but the British Crown still holds the copyright. All PRs
% and issues are reviewed by one of the maintainers and there is no formal
% process for making decisions when conflicts arise.
