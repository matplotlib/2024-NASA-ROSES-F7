...
through the use of the Python programming language.  This development
historically has been largely volunteer work or work that is sponsored
implicitly by specific science projects, but recently there has been explicit
funding.  SPE, shown with a rough schematic in Figure \ref{fig:ecosystem}, has
a core of general purpose domain-agnostic tools, like NumPy~\cite{Harris2020}
and SciPy~\cite{Virtanen2020}. Matplotlib is in the next ring of specialized,
but still domain agnostic tools, and is the most prevalent data visualization
...
specialized plotting.  This layered approach gives scientists and engineers
...
volunteer, community.  Matplotlib has over 1,700 individual contributors to the
code base, with many more individuals having contributed in ways not easily
tracked, such as answering user questions on the mailing list and reporting
issues.

% - TODO mention the new work here?
...
for the string ``matplotlib'' we can thus put a rough lower bound, as
...
project healthy.  \textbf{We propose to split the effort on this grant IN SOME
  WAY between maintenance and new development}.
...
% - TODO re-run this with updated numbers, re-run
...
real-time chat and a Triage Team.
...
Matplotlib has an active real-time chat room 
(\url{https://gitter.im/matplotlib/matplotlib}) chat room that we use 
for general coordination and resolving minor technical discussions.  
While the chat room's history is technically persistent, we treat it 
as transient.  For more in-depth discussions or anything we want a 
record of, we move the conversation to github, the mailing list, or discourse.
